...
Status: on-going 
Tasks: 
1. Altitude sensor random noise: DONE 
2. Vertical velocity random noise: DONE 
=> Trick: the resolution of odeint () must be smaller than the resolution of random. If those resolutions are equal => odeint () crashes due to high variance of the system 
...
altitude_lx, v_list,sps,venting_status,ts = PID (ODEINT_initial_params,time_params,PID_params,control_params,altitude_setpoints,noise)
