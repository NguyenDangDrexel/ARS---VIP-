...
# ------------------------------------------------------------------------------------------------------ 
# PID 
def PID (ODEINT_initial_params: list, time_params:list, PID_params:list, control_params:list, altitude_setpoints:list, noise: list ) -> list: 
    """ 
    ...
    ts = np.linspace(0, simulation_time-1, nsteps-1)           # linear vector space # np.linspace (start,stop,num_points)
...
# time from min to setpoint altitude 
    ...
    for i in tqdm (range(nsteps - 1)):# 0.1 seconds  
        wind_speed = 0 
        u = 0 
        y = odeint(venting, y0, [0, delta_t], args=(u,wind_speed)) 
...
# --------------- estimate velocity for 1 seconds 
        # if i ==0 :
        #     pass
        # else: 
        #     if i % 10 == 0  : 
        #         velocity_second = np.sum (raw_velocity[i-10:i])
        #         velocity_one_second.append (velocity_second) 

# ----------------- altitude: input into the controller 

        estimated_altitude = altitude 
# ------------------------------------------------------------------------------------- 
#                           Create the wind noise 
        ...
        if estimated_altitude < altitude_sp_min: 
            ...
            # velocity_sp = 1 *(avg_velocity- abs ((estimated_altitude) - altitude_sp_min)/tau)  + control_params[1]  # this setpoint change with time 
            ...
            if y[-1][1] >=2: 
                ...
                control_interval = 3 * sensor_pulses
            if i % control_interval == 0:
                # -------------- velocity input into the controller ----------------------------- 

                estimated_velocity = np.sum (raw_velocity[i-control_interval:i]) / control_interval # taking average of the whole control interval 
...
altitude_lx, v_list,sps,venting_status,ts,estimated_altitude,estimated_velocity = PID (ODEINT_initial_params,time_params,PID_params,control_params,altitude_setpoints,noise)
